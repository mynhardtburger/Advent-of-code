...
def apply_multiple_moves(stacks: Stacks, moves: Moves) -> Stacks:
    ...
    for _, (qty, from_stack, to_stack) in enumerate(moves):
        ...
        for _ in range(qty):
            tmp_stacks[to_stack - 1].insert(
                insert_pos, tmp_stacks[from_stack - 1].pop()
            )
...
if __name__ == "__main__":
    ...
    print(
        "Test data single moves stack tops:",
        get_stack_tops(apply_single_moves(stacks, moves)),
    )
    print(
        "Test data multiple moves stack tops:",
        get_stack_tops(apply_multiple_moves(stacks, moves)),
    )

    stacks, moves = read_data(question_data)
    output_stacks = apply_single_moves(stacks, moves)
    print(
        "Question data single moves stack tops:",
        get_stack_tops(apply_single_moves(stacks, moves)),
    )
    print(
        "Question data multiple moves stack tops:",
        get_stack_tops(apply_multiple_moves(stacks, moves)),
    )
